...
const HistoryAndDiagnosticData = ({ parentKey, form, initialData }: OwnProps) => {
  ...
  return (
    <div className={styles.historyAndDiagnosisHypSelect}>
      <Form.Item>
        <Form.Item
          label={intl.get('prescription.historyStep.form.family.label')}
          name={getName(HISTORY_AND_DIAG_FI_KEY.REPORT_HEALTH_CONDITIONS)}
          valuePropName="checked"
          className="noMarginBtm"
        >
          <Checkbox>{intl.get('prescription.historyStep.form.family.checkbox')}</Checkbox>
        </Form.Item>
        <Form.Item
          noStyle
            ...
            getFieldValue(getName(HISTORY_AND_DIAG_FI_KEY.REPORT_HEALTH_CONDITIONS)) ? (
              <Form.Item className={cx('noMarginBtm', styles.healthConditionsWrapper)}>
                <Form.Item {...hiddenLabelConfig} className="noMarginBtm">
                  <ProLabel
                    title={intl.get('prescription.historyStep.form.family.healthCondition.label')}
                    colon
                    requiredMark
                    size="small"
                  />
                  ...
                  rules={[
                    {
                      validator: async (_, conditions: IHealthConditionItem[]) => {
                        if (
                          ...
                          return Promise.reject(new Error(intl.get('prescription.historyStep.form.family.healthCondition.error')));
                        }
                      },
                    },
                  ]}
                        ...
                        {fields.map(({ key, name, ...restField }) => (
                          <Form.Item key={key} {...hiddenLabelConfig} className="noMarginBtm">
                            ...
                            >
                              ...
                              >
                                <Input placeholder={intl.get('prescription.historyStep.form.family.healthCondition.placeholder')} onChange={resetListError} />
                              </Form.Item>
                              <Form.Item
                                {...restField}
                                rules={key > 0 ? defaultFormItemsRules : undefined}
                              ...
                              >
                                <Select placeholder={intl.get('prescription.historyStep.form.family.healthCondition.parentalLink')} onChange={resetListError}>
      ...
      <Form.Item
        label={intl.get('prescription.historyStep.form.family.blood.label')}
      ...
      >
        <Radio.Group>
          ...
          <Radio value={undefined}>{intl.get('na')}</Radio>
        </Radio.Group>
      </Form.Item>
      <Form.Item
      ...
      <Form.Item
        ...
        className="noMarginBtm"
      >
        <Input.TextArea rows={3} placeholder={intl.get('prescription.historyStep.form.family.inputHypothesis.label')} data-cy="InputHypothesis" />
      </Form.Item>
    </div>
  );
};
